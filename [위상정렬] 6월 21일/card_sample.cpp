...
ci ctrl(int row, int col, int dir, vector<vector<int>> &tmp) {
    while (true) {  // 반복
        ...
        if (row < 0 || row >= SIZE || col < 0 || col >= SIZE) { //해당 방향에 카드가 하나도 없다면 그 방향의 가장 마지막 칸으로 이동
            return make_pair(row - dr[dir], col - dc[dir]); // 이동한 좌표 값 리턴
        }
        if (tmp[row][col] != 0) { //누른 키 방향에 있는 가장 가까운 카드
            return make_pair(row, col); // 좌표 값 리턴
        }
    }
}
...
int bfs(int r1, int c1, int r2, int c2, vector<vector<int>> &tmp) {
    ...
    while (!q.empty()) {    // 비어있지 않다면
        int row = q.front().first;  // 열을 큐의 가장 첫 값으로
        int col = q.front().second; // 행을 큐의 가장 첫 값으로
        ...
        for (int i = 0; i < 4; i++) { //컨트롤 입력 이동
            ...
            if (visited[np.first][np.second]) { // 만약 값이 있으면
                continue;   // 넘어감
            }
            visited[np.first][np.second] = dist + 1;    // 아니면 추가
            q.push(np);
        }

        for (int i = 0; i < 4; i++) { //방향키 입력 이동
            int nr = row + dr[i], nc = col + dc[i]; // 매칭할 카드 위치
            if (nr < 0 || nr >= SIZE || nc < 0 || nc >= SIZE || visited[nr][nc]) {  // 만약 0 미만, 사이즈보다 크거나 같으면
                continue;   // 넘어감
            }
            visited[nr][nc] = dist + 1; // 값 1 증가
            q.push({nr, nc});
        }
    }
...
vector<vector<ci>> findCard(vector<vector<int>> &board) {
    ...
    for (int i = 0; i < SIZE; i++) {    // 반복
        for (int j = 0; j < SIZE; j++) {    // 반복
            cnt = max(cnt, board[i][j]);    // 카드의 개수 갱신
            if (board[i][j]) { //카드라면 위치 저장
                card_pos[board[i][j]].push_back({i, j});    // 가능한 카드 위치 저장
            }
        }
    }
